...
let client_id;
try {
    client_id = fs.readFileSync('./ignore/clientID', 'utf8').trim();
} catch (fileErr) { }
let client_secret;
try {
    client_secret = fs.readFileSync('./ignore/secret', 'utf8').trim();
} catch (fileErr) { }
let togetherAPIKey;
try {
    togetherAPIKey = fs.readFileSync('./ignore/togetherAPIKey', 'utf8').trim();
} catch (fileErr) { }
let randomUUID;
try {
    randomUUID = fs.readFileSync('./ignore/randomUUID', 'utf8').trim();
} catch (fileErr) {
    randomUUID = crypto.randomUUID();
    try {
        fs.writeFileSync('./ignore/randomUUID', randomUUID, 'utf8');
    } catch (fileErr2) { }
}
